...
import IesChart from "./IesChart";
import { useNavigate } from "react-router-dom";
import { handleLogout } from "../utils/logout";

export default function Ies() {

const [iesChartData1, setIesChartData1] = useState([]);
const [iesChartData2, setIesChartData2] = useState([]);
    ...
    const fetchFilteredData = async () => {
      try {
        ...
        const [res1, res2 , res3] = await Promise.all([
          axios.get("http://localhost:5000/ieschart", {
            params: {
              question: 'Q8A',
              centres,
              genders,
          ...
          axios.get("http://localhost:5000/ieschart2", {
            params: {
              question: 'Q8B',
              centres,
              genders,
        ...
        setIesChartData1(res1.data);
        setIesChartData2(res2.data);
        setBaseData(res3.data);
      } catch (error) {
...
const brac = iesChartData1.map(d => d.brackets);
const cp = iesChartData1.map(d => d.current_percentage);
const ep = iesChartData2.map(d => d.expected_percentage);
...
/>
          ...
          {/* <div class="w-[23%] text-center flex justify-center items-center">
                        <span class="w-1/4 text-lg font-medium text-secondary" id="base">n={base}</span>   
                       </div> */}
        </div>
    ...
    <div className="row-start-2 row-end-[-2] col-start-1 col-end-[-1]">
              ...
              <div className="w-[90%] h-[90%] overflow-scroll">
                ...
                <table id="Table" className="w-full bg-white text-sm text-left rtl:text-right">
                                <thead className=" w-full text-white bg-white sticky top-0">
                                    ...
                                    <th className="min-w-[10vw] px-3 py-3 border-2 border-white text-center text-[1.25rem]" > Bracket </th>
                                    <th className="min-w-[10vw] px-3 py-3 border-2 border-white text-center text-[1.25rem]" >Perception of current inflation</th>
                                    <th className="min-w-[10vw] px-3 py-3 border-2 border-white text-center text-[1.25rem]" >One year ahead inflation expectations</th>
                                    </tr>
                                </thead>
                                <tbody>
                                {brac.map((bracket, index) => (
                                    <tr key={index} className="border-2 border-bordergrey">
                                    <td className="min-w-[8vw] border-l-2 border-backgrey px-3 py-3 text-[0.95rem] text-black uppercase text-center">
                                        {bracket}
                                    </td>  
                                    <td className="min-w-[8vw] border-l-2 border-backgrey px-3 py-3 text-center">
                                        {cp[index] ?? 0}%
                                    </td> 
                                    <td className="min-w-[8vw] border-l-2 border-backgrey px-3 py-3 text-center">
                                        {ep[index] ?? 0}%
      ...
      <div className="absolute bottom-0 w-full">
                ...
                <nav className="w-full flex justify-center items-center flex-wrap  bg-[#012150] py-2 px-2">
                    ...
                    <Link to="/ie" className="text-white text-[0.95rem] font-semibold px-3 py-1 border-r-2 border-white bg-[#012150] ">
                        Income Economic
                    </Link>
                    <Link to="/os" className="text-white text-[0.95rem] font-semibold px-3 py-1 border-r-2 border-white bg-[#012150] ">
                        Overall Spending
                    </Link>
                    <Link to="/es" className="text-white text-[0.95rem] font-semibold px-3 py-1 border-r-2 border-white bg-[#012150] ">
                        Essential Spending
                    </Link>
                    <Link to="/nes" className="text-white text-[0.95rem] font-semibold px-3 py-1  border-white bg-[#012150] ">
                        Non Essential Spending
                    </Link>
                    <Link to="/ies" className="text-white  text-[0.95rem] font-semibold px-3 py-2 border-4 border-white bg-[#c0860c] rounded-full -translate-y-7">
